...
const BASE = 'http://localhost:3000/api/mpz/view/sphere'
...
describe('POST /api/mpz/view/sphere', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
    vi.restoreAllMocks()
  })
  ...
  it('production → 404', async () => {
    ...
    const res = await POST(
      postRequest({ slug: 'daz', startYaw: 0, startPitch: 0 }, { cookie: SECRET }),
    )
  ...
  it('development ohne Cookie → 401', async () => {
    ...
    vi.stubEnv('SN_MPZ_STUDIO_SECRET', SECRET)
    const res = await POST(postRequest({ slug: 'daz', startYaw: 0, startPitch: 0 }))
