...
const BASE = 'http://localhost:3000/api/mpz/hotspots/flat'
...
describe('POST /api/mpz/hotspots/flat', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
    vi.restoreAllMocks()
  })
  ...
  it('production → 404', async () => {
    ...
    const res = await POST(
      postRequest(
        { slug: 'kunst', hotspotId: 'hs-demo', x: 0.5, y: 0.5 },
        { cookie: SECRET },
      ),
    )
  ...
  it('development ohne Cookie → 401', async () => {
    ...
    vi.stubEnv('SN_MPZ_STUDIO_SECRET', SECRET)
    const res = await POST(
      postRequest({ slug: 'kunst', hotspotId: 'hs-demo', x: 0.5, y: 0.5 }),
    )
