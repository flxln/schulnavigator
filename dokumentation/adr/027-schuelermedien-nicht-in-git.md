...
- **Media- und Audio-Routen** (`/media/*`, `/api/dialog/*`, `/api/coach/*`): bei fehlendem Entry-Cookie **403** mit leerem Body — kein 307-Redirect wie in der Middleware, damit `<video>`/`<img>` keine HTML-Antwort statt Binärdaten erhalten. `Cache-Control: private, max-age=3600` (kein `public`).
