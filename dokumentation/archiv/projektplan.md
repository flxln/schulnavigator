...
> **Eingefroren** (2026-06-20). Nicht mehr pflegen. Offene Punkte: [`planung/offen.md`](../planung/offen.md) · GitHub Issues · Phasenplan nur noch als Historie.
...
- [x] Deployment auf MPZ-Server (Coolify) — **Live:** `https://schulnavigator.mpz.schule` (GitHub #16 geschlossen). Runbook [`anleitungen/fuer-entwickler.md`](../anleitungen/fuer-entwickler.md), Go-Live-Härtung (`robots`, `noindex`, `/eintritt`-Platzhalter); Wildcard-DNS `*.mpz.schule` → VPS; Dockerfile: `npm ci --include=dev` (Build mit `NODE_ENV=production` in CI)
...
- Deploy-Link für Sten/Tina: **`https://schulnavigator.mpz.schule`** (Coolify, HTTPS — Schritte und Smoke-Tests in [`anleitungen/fuer-entwickler.md`](../anleitungen/fuer-entwickler.md))
...
- [x] **ADR-017** — Externe Medien & Hotspot-Marker ([Umsetzungsplan](./projektmanagement/2026-06-10-externe-medien-hotspot-marker-plan.md), Epic **#97**):
...
5. ~~**Projektsetup (Phase 1):** Next.js in `app/`, Dockerfile, `/api/health`~~ — #9–#12 erledigt; ~~**Deploy:** MPZ/Coolify (#16)~~ — live `schulnavigator.mpz.schule`, Runbook + Härtung + Docker-Buildfix im Repo
