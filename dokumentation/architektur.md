...
**Ist-Zustand:** Mobile-first — Haupt-Routen `mx-auto max-w-lg` (≈ 512 px); Raum-Hero `min(52–58vh, 340–400px)`; Viewer-Konstante `min(50vh, 360px)` ([`constants.ts`](../app/lib/raum-viewer/constants.ts)). Viewport-Meta `device-width`, `initialScale: 1`, `userScalable: false` (Pinch-Zoom gesperrt; OS-Schriftgröße und Desktop-Browser-Zoom bleiben möglich) ([#56](./github-project/issues-phase-2.md)).
